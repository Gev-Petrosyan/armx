...
<body>
    ...
    <main>
        ...
        <div class="company-info">
            <form method="POST" action="{{route("updateCity")}}" enctype="multipart/form-data" style="display:flex">
                @csrf
                ...
                <div class="product-image">
                    <label class="input-logo" for="product_image" tabIndex="0" style="margin-bottom: 25px">
                        @if ($city->image)
                            <img src="{{asset("storage/city/" . $city->image)}}" style="width: 100%" id="product_img" alt="img">    
                        @else
                    ...
                    <a href="{{route("deleteCity", $city->id)}}" style="margin-top:15px;background:red;width:100%">Удалить</a>
                </div>
                <div class="product-inputs">
                    <div class="input-group">
                        <label for="name">Название города</label>
                        <input type="name" id="city" name="city" value="{{$city->city}}" required>
                    </div>
                </div>
            </form>
        </div>
